...
const SubsubcategoryTemplate = ({ data, location, search }) => {
  ...
  nodes.forEach(item => {
    ...
    if (tip_filter) {
      const found = filterItems.tip_filter.values.find(el => el === tip_filter)
      if (!found) {
...
export default withLocation(SubsubcategoryTemplate)

export const query = graphql`
  query SubProductQuery($id: Int!) {
    allStrapiProduct(
      filter: { subsubcategories: { elemMatch: { id: { eq: $id } } } }
    ) {
      nodes {
        strapiId
        name
